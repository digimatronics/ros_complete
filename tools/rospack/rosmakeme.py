...
ret = run_process('cmake %s ..' % win32_generator_flag,
        msg_on_fail='[rosbuild] CMake failed; trying to clean and start over',
        working_dir='build')[0]
if ret != 0:
    os.rmdir('build')
    mkdir('build', make_parents=True)
    run_process('cmake %s ..' % win32_generator_flag, working_dir='build')
run_process('${MAKE} %s' % envvar('PARALLEL_JOBS'), working_dir='build')
